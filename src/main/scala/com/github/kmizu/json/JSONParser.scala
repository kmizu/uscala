...
import com.github.kmizu.json._
import JSONValue._
...
object JSONParser extends RegexParsers {
  ...
  def expression: Parser[JSONValue] = SPACING ~> nvalue <~ SPACING
  ...
  private lazy val nvalue: Parser[JSONValue] = doubleLiteral | integerLiteral | stringLiteral | objectLiteral | arrayLiteral | nullLiteral | booleanLiteral | ident | CL(LPAREN) ~>expression<~ RPAREN

  private lazy val nullLiteral: Parser[JNull.type] = NULL <~ SPACING_WITHOUT_LF ^^ { _ => JNull}

  //intLiteral ::= ["1"-"9"] {"0"-"9"}
  private lazy val integerLiteral : Parser[JLong] = (opt("+" | "-") ~
    ( ("0x" ~> """[0-9a-fA-F][0-9a-fA-F]*|0""".r  ^^ { case value =>
      JavaLong.parseLong(value, 16)
    })
      | ("""[1-9][0-9]*|0""".r ^^ { case value =>
      JavaLong.parseLong(value)
    })
      ) ^^ { case sign ~ value =>
  ...
  private lazy val doubleLiteral: Parser[JDouble] = (opt("+" | "-") ~ "([1-9][0-9]*|0)\\.[0-9]*".r ^^ {
    case sign ~ value =>
  ...
  private lazy val booleanLiteral: Parser[JBoolean] = (TRUE ^^ { _ => true }| FALSE ^^ { _ => false}) ^^ {
    case true => JBoolean(true)
    case false => JBoolean(false)
  }

  //stringLiteral ::= "\"" ((?!")(\[rntfb"'\\]|[^\\]))* "\""
  private lazy val stringLiteral : Parser[JString] =
    (("\"" ~> ( """((?!("|#\{))(\\[rntfb"'\\]|[^\\]))+""".r ).* <~ "\"") ^^ { values =>
      JString(values.mkString)
    }) <~ SPACING_WITHOUT_LF

  private lazy val arrayLiteral: Parser[JArray] = ((CL(LBRACKET) ~> (repsep(CL(expression), SEPARATOR) <~ opt(SEPARATOR)) <~ RBRACKET) ^^ {
    case contents => JArray(contents)
  }) <~ SPACING_WITHOUT_LF

  private lazy val objectLiteral: Parser[JObject] =  ((CL(LBRACE) ~> (repsep(CL((stringLiteral | ident) ~ opt(COLON) ~ expression), SEPARATOR) <~ opt(SEPARATOR)) <~ RBRACE) ^^ {
    case contents => JObject(contents.map { case k ~ _ ~ v  => k.value -> v}.toMap)
  }) <~ SPACING_WITHOUT_LF

  private lazy val ident :Parser[JString] = ( """[A-Za-z_][a-zA-Z0-9]*""".r ^^ {
    case value  => JString(value)
  }) <~ SPACING_WITHOUT_LF

  def parse(str:String): JSONValue = parseAll(expression, str) match {
